...
namespace CaptureAndSend
{
	...
	public class FtpClient
	{
		...
		/// <returns></returns>
		//
		// Done when 3 digits followed by space
		//
		private bool isDone( byte []b, long read )
		{
			if ( read >= 4 
				&& Char.IsDigit((char)b[0]) 
				&& Char.IsDigit((char)b[1])
				&& Char.IsDigit((char)b[2])
				&& Char.IsWhiteSpace((char)b[3])
				)
			{
				return true;
			}
		...
		private string readLine()
		{
			...
			while (true)
			{
				...
				this.message  += ASCII.GetString(buffer, 0, bytes);
				if (bytes < buffer.Length && isDone(buffer,bytes) )
				{
					break;
				}
			}
			...
			char[] seperator = {'\n'};
			string[] mess = this.message.Split(seperator);

			if (this.message.Length > 2)
			{
				this.message = mess[mess.Length-2];
			} 
			else
			{
				this.message = mess[0];
			...
			{
				this.message += " Corrupt reply from ftp server, (Length < 4)";
			}
